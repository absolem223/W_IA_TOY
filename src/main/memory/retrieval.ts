...
export interface MemoryUsedItem {
  ...
  label: string
  score: number
  content?: string // full text value — required for actual injection into the prompt
}
...
export function assembleMemoryPreamble(
  ...
  if (profile && totalChars + profile.length < maxChars) {
    ...
    for (const [key, entry] of Object.entries(semantic.profile)) {
      ...
      const val = Array.isArray(entry.value) ? entry.value.join(', ') : entry.value
      usedMemories.push({ type: 'profile', label: key.replace(/_/g, ' '), score: 1, content: val })
    }
  ...
  const snippets = retrieveRelevant(currentInput, semantic, vaultEntries, episodic || { version: 1, episodes: [] }, log)
  if (snippets.length > 0) {
    ...
    for (const s of snippets) {
      ...
      usedMemories.push({ type: s.source, label: s.label, score: Math.round(s.score * 100) / 100, content: s.content })
    }
